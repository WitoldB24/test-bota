...
def start(message):
    ...
    bot.send_message(message.chat.id, 'Здравствуйте, я Бот для Конвертинга валют и проконвертирую твою валюту'.format(message.from_user),
                     reply_markup=markup)
...
def bot_message(message):
    if message.chat.type == 'private':
        if message.text == 'Помощь':
        ...
        elif message.text == 'API предоставили':
            ...
            bot.send_message(message.chat.id, 'Бот использует бесплатный API, поэтому возможности ограничены.')
        else:
            try:
